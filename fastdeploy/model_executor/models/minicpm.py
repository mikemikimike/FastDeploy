...
class MiniCPMDecoderLayer(nn.Layer):
    ...
    def __init__(
        ...
        self.self_attn = MiniCPMAttention(fd_config, layer_id, prefix=f"{prefix}.self_attn")
        ...
        self.mlp = MiniCPMMLP(fd_config, prefix=f"{prefix}.mlp")
...
class MiniCPMPretrainedModel(PretrainedModel):
    ...
    def _get_tensor_parallel_mappings(cls, config: ModelConfig, is_split=True):
        ...
        def get_tensor_parallel_split_mappings():
            return [
                # Column Linear
                (["embed_tokens.weight"], fn, [0]),
                (["q_proj.weight"], fn, [0]),
                (["k_proj.weight"], fn, [0]),
                (["v_proj.weight"], fn, [0]),
                (["gate_proj.weight"], fn, [0]),
                (["up_proj.weight"], fn, [0]),
                (["lm_head.weight"], fn, [0]),
                # Row Linear
                (["o_proj.weight"], fn, [1]),
                (["down_proj.weight"], fn, [1]),
            ]

        return get_tensor_parallel_split_mappings()
